...
namespace EdriveAuto.GenericService.Interfaces;

public interface IBaseService<T> where T : class, IBaseDTOModel, new()
{
    Task<PaginatedList<T>> GetAllAsync(PaginatedList<T> pagination);
		
    Task<PaginatedList<T>> GetAllAsync(int pageNumber, int pageSize, string orderByString, bool isAsc = true);
